...
# ==========================================
cat > "$TARGET_DIR/.antigravity/project-context.md" << CONTEXTEOF
# Project Context
# ===============
# Auto-generated by ANTIGRAVITY-STARTER
# Do not edit manually — run init-project.sh to regenerate
...
| Layer | Technology |
|-------|-----------|
$(if [ "$FRONTEND" = "nextjs" ]; then echo "| Frontend | Next.js + TypeScript + Tailwind CSS |"; fi)
$(if [ "$BACKEND" = "fastapi" ]; then echo "| Backend | FastAPI + Python |"; fi)
$(if [ "$DB_TYPE" != "none" ]; then echo "| Database | $DB_TYPE |"; fi)
$(if [ "$AI_PROVIDER" != "none" ]; then echo "| AI | $AI_PROVIDER |"; fi)
...
$(if [ "$NEED_AUTH" = "y" ] || [ "$NEED_AUTH" = "Y" ]; then echo "- **Auth:** Enabled"; fi)
$(if [ "$NEED_TELEGRAM" = "y" ] || [ "$NEED_TELEGRAM" = "Y" ]; then echo "- **Telegram:** Enabled"; fi)
$(if [ "$NEED_GOOGLE" = "y" ] || [ "$NEED_GOOGLE" = "Y" ]; then echo "- **Google:** Enabled"; fi)

## Deployment

- **Type:** ${DEPLOY_TYPE:-none}

## Project Structure

\`\`\`
.antigravity/   # AI agent instructions and rules
apps/           # Application code
packages/       # Shared packages
...
- Read .antigravity/AGENTS.md for AI development instructions
- Follow .antigravity/rules/ for coding standards
- Use .antigravity/workflows/ for development workflows
- Check .antigravity/project.config.json for project configuration
CONTEXTEOF
echo -e "  ${GREEN}${NC} .antigravity/project-context.md — project context generated"
